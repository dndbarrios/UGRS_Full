		...
		select @DocNumSMAjuste= '{DocNum}'
              ...
              select @CamtDocRev1 = (select count(*) from OMRV t0 where  t0.U_DocNumSalida =@RevFolFind and t0.DocDate=@DocDateSMAjuste)
              select @CamtDocRev2 = (select count(*) from OMRV t0 where  t0.U_DocNumSalida = @RevFolFind and t0.DocDate>@DocDateSMAjuste)
